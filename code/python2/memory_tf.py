...
class DKVMN_Memory():
	...
	def write(self, value_matrix, correlation matrix, qa_embedded):
		'''
		...
		add_vector = operations.linear(qa_embedded, self.memory_state_dim, name=self.name+'Add_Vector')
		# [batch size, memory state dim(d_v)]
...
class DKVMN():
	def __init__(self, memory_size, memory_key_state_dim, memory_value_state_dim, init_memory_key, init_memory_value, name='DKVMN'):
		...
		self.key = DKVMN_Memory(self.memory_size, self.memory_key_state_dim, name='key_matrix_')
		self.value = DKVMN_Memory(self.memory_size, self.memory_value_state_dim, name='value_matrix_')
