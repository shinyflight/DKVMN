...
class MODEL(object):
    ...
    def sym_gen(self):
        ### transpose?
        q_data = tf.placeholder(tf.float32, shape=[self.seqlen, self.batch_size])
        qa_data = tf.placeholder(tf.float32, shape=[self.seqlen, self.batch_size])
        target = tf.placeholder(tf.float32, shape=[self.seqlen, self.batch_size])

        """
        q_data = tf.placeholder(tf.float32, shape=[self.batch_size, self.seqlen])
        qa_data = tf.placeholder(tf.float32, shape=[self.batch_size, self.seqlen])
        target = tf.placeholder(tf.float32, shape=[self.batch_size, self.seqlen])
        """
        ...
        init_memory_key = tf.Variable(
            tf.randon_normal([self.batch_size, self.memory_size, self.memory_key_state_dim], stddev=0.5),
            name='init_memory_key_weight')
        init_memory_value = tf.Variable(
            tf.random_noraml([self.batch_size, self.memory_size, self.memory_value_state_dim], stddev=0.5),
        ...
        q_embedding = tf.Variable(tf.random_uniform([self.n_question + 1, self.q_embed_dim], -1.0, 1.0))
        q_embed_data = tf.nn.embedding_lookup(q_embedding, q_data)
        slice_q_embed_data = tf.split(q_embed_data, self.seqlen)

        qa_embedding = tf.Variable(tf.random_uniform([self.n_question * 2 + 1, self.qa_embed_dim], -1.0, 1.0))
        qa_embed_data = tf.nn.embedding_lookup(qa_embedding, qa_data)
        slice_qa_embed_data = tf.split(qa_embed_data, self.seqlen)
        ...
        all_read_value_content = tf.concat(*value_read_content_l)
        input_embed_content = tf.concat(*input_embed_l)
        input_embed_content = tf.layers.dense(input_embed_content, 50, name="input_embed_content")
        input_embed_content = tf.nn.tanh(input_embed_content, name="input_embed_content_tanh")

        read_content_embed = tf.layers.dense(tf.concat([all_read_value_content, input_embed_content], axis=1),
                                             self.final_fc_dim, name="read_content_embed")
        read_content_embed = tf.nn.tanh(read_content_embed, name="read_content_embed_tanh")

        pred = tf.layers.dense(read_content_embed, 1, name="final_fc")
        pred_prob = logistic_regression_mask_output(data=tf.reshape(pred, [-1]),
                                                    label=tf.reshape(target, [-1]),
                                                    ignore_label=-1., name="final_predict")
